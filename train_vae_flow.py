...
parser.add_argument("--max_beta", type=float, default=1.0, metavar="MB", help="max beta for warm-up")
...
def run(args, kwargs):
    ...
    elif args.flow == "cnf_rank_stl":
        snap_dir = snap_dir + "_rank_" + str(args.rank) + "_stl_" + args.dims + "_num_blocks_" + str(args.num_blocks)
    ...
    # LOAD DATA
    # ==================================================================================================================
    train_loader, val_loader, test_loader, args = load_dataset(args, debug=args.debug, **kwargs)
    ...
    if not args.evaluate:
        ...
        if args.flow == "no_flow":
        ...
        elif args.flow == "cnf_stl":
            model = CNFVAE.CNFVAESTL(args)
            ...
            model = CNFVAE.LowRankCNFVAESTL(args)
        else:
        ...
        optimizer = optim.Adamax(parameters, lr=args.learning_rate, eps=1.0e-7)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            t_start = time.time()
            tr_loss = train(epoch, train_loader, model, optimizer, args, logger, writer=writer)
            train_loss.append(tr_loss)
            train_times.append(time.time() - t_start)
            logger.info("One training epoch took %.2f seconds" % (time.time() - t_start))

            v_loss, v_bpd = evaluate(val_loader, model, args, logger, epoch=epoch, writer=writer)
